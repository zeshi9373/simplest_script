...
type ScriptConfig struct {
	...
	ExecCmd             string `gorm:"column:exec_cmd;NOT NULL" json:"exec_cmd"`                                     // 执行指令
	Params              string `gorm:"column:params;NOT NULL" json:"params"`                                         // 参数
	Topic               string `gorm:"column:topic;NOT NULL" json:"topic"`                                           // 主题
	GroupId             string `gorm:"column:group_id;NOT NULL" json:"group_id"`                                     // 消费组
	...
	MaxProgress         int    `gorm:"column:max_progress;NOT NULL;default:0" json:"max_progress"`                   // 最大消费者数量
	ProgressLagLimit    int    `gorm:"column:progress_lag_limit;NOT NULL;default:0" json:"progress_lag_limit"`       // 消息堆积阈值
	ProgressAvgMsgcount int    `gorm:"column:progress_avg_msgcount;NOT NULL;default:0" json:"progress_avg_msgcount"` // 新增单个消费者处理的消息数
	...
	IsLog               int    `gorm:"column:is_log;NOT NULL;default:0" json:"is_log"`                               // 定时任务是否记日志
